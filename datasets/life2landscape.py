...
class Life2LandscapeDataset(chainer.dataset.DatasetMixin):
...
if __name__ == "__main__":
    ...
    dirname_to_label = {}
    with open('dirname_to_label_life2landscape.txt', 'r') as f:
        for line in f:
    ...
    if preprocess_val:
        ...
        filenames = glob.glob(root_path_val + '/*.jpg')
        for filename in filenames:
            filename = filename.split('/')
            image_name = filename[-1].strip('.jpg')
